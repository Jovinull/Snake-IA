...
class Agent:
    ...
    def __int__(self):
        ...
        self.memory = deque(maxlen=MAX_MEMORY) # Se excedemos a memória ele ira remover os itens da esquerda
    ...
    def remember(self, state, action, reward, next_state, done):
        self.memory.append((state, action, reward, next_state, done)) # popleft if MAX_MEMORY if reached
...
def train():
    ...
    while True:
        ...
        if done:
            # train long memory (replay memory / expirence replay)
            game.reset()
            agent.n_games += 1
